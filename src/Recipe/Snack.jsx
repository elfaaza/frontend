...
const Snack = ({ searchTerm }) => {
  ...
  useEffect(() => {
    ...
    console.log("All Categories:", data.data?.map(r => r.category));

        const filteredData = data.data?.filter(recipe => recipe.category && recipe.category == 6) || [];
    console.log("Filtered Recipes (Category 6):", filteredData);

        setRecipes(filteredData);
        const newTotalPages = Math.ceil(filteredData.length / data.meta?.per_page || 6) || 1;
